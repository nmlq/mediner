...
def test_convert_csv_to_label_studio_with_file_output(mock_input_csv, tmp_path):
    temp_output_path = tmp_path / "temp.output.json"
    assert not os.path.isfile(str(temp_output_path))
    assert commands.convert_csv_to_label_studio(mock_input_csv, "ReportText", output_filename = str(temp_output_path))
